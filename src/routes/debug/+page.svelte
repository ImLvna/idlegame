...
<Fade visible={unlocked} invert>
	<Themable bg={800} class="size-full rounded-xl">
		<PinInput.Root
			...
			placeholder={'0'}
			type={'text'}
			class="size-full gap-3 flex flex-row items-center justify-center"
		>
			{#each Array.from({ length: DEBUG_PIN.length }, (_, i) => i) as i}
				<Themable
					bg={900}
					text={'white'}
					class="h-28 w-20 flex flex-row items-center justify-center"
				>
					<PinInput.Input
						class="size-full rounded-input text-center bg-inherit placeholder-shown:border-border-input"
					/>
				</Themable>
			{/each}
		</PinInput.Root>
